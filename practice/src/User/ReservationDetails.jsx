...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  const getParticipantId = (participant) => {
    return participant.id_number || participant.idNumber || participant._id;
  };

  // Helper function to get participant name consistently
  ...
  const getRoomImage = () => {
    ...
    // Exact match first
    if (roomName) {
      const exactMatch = availableRoomImages.find(img => 
        img.name.toLowerCase() === roomName.toLowerCase()
      );
      if (exactMatch) return exactMatch;
      
      // Then partial matches
      const partialMatch = availableRoomImages.find(img => 
  ...
  const originalEndTime = localReservation ? new Date(localReservation.endDatetime) : null;
  const currentEndTime = extendedEndTime || originalEndTime;

  const minExtensionTime = currentEndTime ? new Date(currentEndTime) : null;
  const maxExtensionTime = localReservation?.maxExtendedEndDatetime 
    ? new Date(localReservation.maxExtendedEndDatetime)
    : (currentEndTime ? new Date(currentEndTime.getTime() + 4 * 60 * 60 * 1000) : null); // 4 hours max by default
  ...
  const handleRemoveParticipant = async (participant) => {
    ...
    try {
      const response = await axios.post(
        ...
        {
          reservationId: localReservation._id,
          participantIdNumber: participantId
        }
      );
      ...
      <div className="ml-0 lg:ml-[250px] p-4 lg:p-8 flex items-center justify-center min-h-[calc(100vh-50px)]">
        <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-md text-center">
          ...
          <button
            ...
            className="bg-[#CC0000] text-white px-5 py-3 rounded-lg hover:bg-red-700 transition-colors cursor-pointer w-full sm:w-auto"
          >
            Back to Dashboard
          </button>
        </div>
  ...
  return (
    <main className="ml-0 lg:ml-[250px] w-full lg:w-[calc(100%-250px)] flex flex-col min-h-screen bg-gray-50 pb-safe">
      {/* Header */}
      <header className="text-black px-4 lg:px-6 h-[60px] flex items-center justify-between shadow-sm bg-white sticky top-0 z-40">
        <h1 className="text-lg md:text-xl lg:text-2xl font-bold tracking-wide">Reservation Details</h1>
        <button 
          onClick={() => setView("dashboard")}
          className="text-sm text-gray-500 hover:text-gray-700 flex items-center cursor-pointer min-h-[44px] px-3 rounded-lg hover:bg-gray-100 transition-colors"
        >
      ...
      <div className="p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-3 sm:p-4 lg:p-6 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800 mb-3 sm:mb-4 pb-2 border-b border-gray-100">Reservation Information</h2>
          ...
          {hasPendingExtension && (
            <div className="mb-3 sm:mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-4">
              <div className="flex items-center">
                ...
                <span className="text-yellow-800 font-medium text-sm sm:text-base">Extension Request Pending Approval</span>
              </div>
            </div>
          )}
          ...
          {localReservation.extensionStatus === "Approved" && (
            <div className="mb-3 sm:mb-4 bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4">
              <div className="flex items-center">
                ...
                <span className="text-green-800 font-medium text-sm sm:text-base">Extension Approved</span>
              </div>
            </div>
          )}
          ...
          {hasRejectedExtension && (
            <div className="mb-3 sm:mb-4 bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4">
              <div className="flex items-center">
                ...
                <span className="text-red-800 font-medium text-sm sm:text-base">Extension Request Declined</span>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5">
            ...
            <div className="space-y-1">
              ...
              <div className="p-3 rounded-lg bg-gray-50 border border-gray-200">
                <p className="font-medium text-gray-800 text-sm sm:text-base">{formatDateOnly(localReservation.datetime)}</p>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="p-3 rounded-lg bg-gray-50 border border-gray-200">
                <p className="font-medium text-gray-800 text-sm sm:text-base">{localReservation.roomName}</p>
              </div>
            </div>
          ...
          <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-200">
            <h3 className="text-sm font-medium text-gray-700 mb-2 sm:mb-3 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              Current Schedule
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
                ...
                <p className="font-medium text-gray-800 text-sm sm:text-base">{formatTimeOnly(localReservation.datetime)}</p>
              </div>
              <div className={`p-3 rounded-lg border ${extendedEndTime ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                ...
                <p className={`font-medium text-sm sm:text-base ${extendedEndTime ? 'text-green-600' : 'text-gray-800'}`}>
                  {formatTimeOnly(currentEndTime)}
                  {extendedEndTime && <span className="text-xs ml-1 sm:ml-2 text-green-600">(Extended)</span>}
                </p>
              </div>
            </div>
          </div>
        ...
        {roomImage && (
          <div className="bg-white rounded-xl shadow-sm p-3 sm:p-4 lg:p-6 border border-gray-100">
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              />
              <p className="font-semibold text-base lg:text-lg mt-3 sm:mt-4 bg-gray-100 px-3 sm:px-4 py-2 rounded-lg border border-gray-200 text-center w-full max-w-2xl">
        ...
        {localReservation.participants && localReservation.participants.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm p-3 sm:p-4 lg:p-6 border border-gray-100">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 sm:mb-4 pb-2 border-b border-gray-100 gap-2">
              ...
              {isMainReserver && ["Pending", "Approved"].includes(localReservation.status) && (
                <button
                  ...
                  className="px-3 sm:px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center justify-center cursor-pointer text-sm min-h-[44px] w-full sm:w-auto"
                >
                      ...
                      {isMainReserver && ["Pending", "Approved"].includes(localReservation.status) && (
                        <td className="px-3 py-2 lg:px-4 lg:py-3 whitespace-nowrap text-xs lg:text-sm">
                          {i !== 0 ? (
                            ...
                            <span className="text-gray-400 text-xs italic">Main Reserver</span>
                          )}
                        </td>
                      )}
                    </tr>
            ...
            <>
              {localReservation.status === "Pending" && (
                <>
                  <button
                    ...
                    className="w-full xs:w-auto px-4 lg:px-5 py-3 sm:py-2 lg:py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer text-sm lg:text-base min-h-[44px]"
                  >
                  ...
                  <button
                    ...
                    className="w-full xs:w-auto px-4 lg:px-5 py-3 sm:py-2 lg:py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer text-sm lg:text-base min-h-[44px]"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              ...
              {localReservation.status === "Approved" && (
                <button
                  ...
                  className="w-full xs:w-auto px-4 lg:px-5 py-3 sm:py-2 lg:py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer text-sm lg:text-base min-h-[44px]"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
              ...
              {localReservation.status === "Ongoing" && (
                <>
                  <button
                    ...
                    className="w-full xs:w-auto px-4 lg:px-5 py-3 sm:py-2 lg:py-2.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer text-sm lg:text-base min-h-[44px]"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
            ...
            <>
              {["Pending", "Approved"].includes(localReservation.status) && (
                <button
                  ...
                  className={`w-full xs:w-auto px-4 lg:px-5 py-3 sm:py-2 lg:py-2.5 rounded-lg transition-colors flex items-center justify-center cursor-pointer text-sm lg:text-base min-h-[44px] ${
              ...
              {canRequestExtension && (
                <button
                  ...
                  className="w-full xs:w-auto px-4 lg:px-5 py-3 sm:py-2 lg:py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer text-sm lg:text-base min-h-[44px]"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 lg:h-5 lg:w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 mx-2">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              ...
              <button
                ...
                disabled={cancelling}
                className="w-full sm:w-auto px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer font-medium min-h-[44px]"
              >
                {cancelling ? (
                  <>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 mx-2">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              ...
              <button
                ...
                disabled={processingAction === "end-early"}
                className="w-full sm:w-auto px-6 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer font-medium min-h-[44px]"
              >
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 mx-2">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              ...
              <button
                ...
                disabled={processingAction === "start"}
                className="w-full sm:w-auto px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer font-medium min-h-[44px]"
              >
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 mx-2">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              ...
              <button
                ...
                disabled={processingAction === "approve"}
                className="w-full sm:w-auto px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer font-medium min-h-[44px]"
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                    ...
                    <p className="text-xs text-purple-600 mt-1">
                      New end time: {currentEndTime ? formatTimeOnly(new Date(currentEndTime.getTime() + (extensionHours * 60 + extensionMinutes) * 60000)) : "N/A"}
                    </p>
                  </div>
                </div>
              )}
              ...
              {extensionType === "custom" && (
                <div>
                  ...
                  <input
                    ...
                    min={minExtensionTime ? formatDateTimeForInput(minExtensionTime) : ""}
                    max={maxExtensionTime ? formatDateTimeForInput(maxExtensionTime) : ""}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  />
                  {maxExtensionTime && (
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100] p-2 sm:p-4">
          <div className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 mx-2">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to remove <strong>{getParticipantName(participantToRemove)}</strong> (ID: {getParticipantId(participantToRemove)}) from this reservation? 
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              ...
              <button
                ...
                disabled={processingParticipantAction === `remove-${getParticipantId(participantToRemove)}`}
                className="w-full sm:w-auto px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors flex items-center justify-center cursor-pointer font-medium min-h-[44px]"
              >
                {processingParticipantAction === `remove-${getParticipantId(participantToRemove)}` ? (
                  <>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                    Removing...
                  </>
            ...
            <div className="space-y-4 mb-6">
              ...
              {availableUsers.length > 0 && (
                <div className="max-h-60 overflow-y-auto border border-gray-200 rounded-lg">
                  {availableUsers.map((user) => (
                    <div
                      key={getParticipantId(user)}
                      className={`p-3 border-b border-gray-100 cursor-pointer hover:bg-gray-50 ${
                        selectedUser && getParticipantId(selectedUser) === getParticipantId(user) ? 'bg-indigo-50 border-indigo-200' : ''
                      }`}
                      onClick={() => setSelectedUser(user)}
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="font-medium text-gray-800 text-sm">{getParticipantName(user)}</p>
                          <p className="text-xs text-gray-600">ID: {getParticipantId(user)}</p>
                        </div>
              ...
              {selectedUser && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4">
                  ...
                  <p className="font-medium text-sm">{getParticipantName(selectedUser)} (ID: {getParticipantId(selectedUser)})</p>
                  <p className="text-xs text-green-700">{selectedUser.course} - {selectedUser.department}</p>
                </div>
              )}
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-200 mx-2">
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-gray-200">
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                <div>
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-800">Manage Participants</h3>
                  <p className="text-xs sm:text-sm text-gray-600 mt-1">Add or remove participants from this reservation</p>
                </div>
              </div>
              <button
                onClick={() => setShowManageParticipants(false)}
                className="text-gray-400 hover:text-gray-600 cursor-pointer p-1 hover:bg-gray-100 rounded-full transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-full text-xs sm:text-sm font-medium">
                ...
                <div className="space-y-2 sm:space-y-3 max-h-60 sm:max-h-80 overflow-y-auto pr-1 sm:pr-2">
                  {localReservation.participants.length > 0 ? (
                    ...
                    <div className="text-center py-6 sm:py-8 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <p className="text-gray-500 font-medium text-sm sm:text-base">No participants added yet</p>
                      <p className="text-xs sm:text-sm text-gray-400 mt-1">Add participants using the form on the right</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-2 rounded-lg font-medium transition-all text-sm sm:text-base min-h-[44px] ${
                      canAddMoreParticipants
                ...
                {!canAddMoreParticipants ? (
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 sm:p-4">
                    <div className="flex items-start space-x-2 sm:space-x-3">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5 text-orange-500 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                      </svg>
                      <div>
                  ...
                  <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-4 sm:p-6 text-center hover:border-gray-400 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 sm:h-12 sm:w-12 mx-auto text-gray-400 mb-2 sm:mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                    <p className="text-gray-600 font-medium mb-1 text-sm sm:text-base">Add participants to this reservation</p>
                    <p className="text-xs sm:text-sm text-gray-500">Click "Add New" to search for and add participants</p>
            ...
            <div className="flex justify-end pt-3 sm:pt-4 border-t border-gray-200">
              <button
                onClick={() => setShowManageParticipants(false)}
                className="w-full sm:w-auto px-6 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors cursor-pointer font-medium shadow-sm min-h-[44px]"
              >
                Close
              </button>
            </div>
