...
function ReservationDetails({ reservation, setView, refreshReservations, user }) {
  ...
  // Helper function to check if participant is the main reserver
  const isMainReserverParticipant = (participant) => {
    if (!localReservation || !localReservation.userId || !participant) return false;
    
    const reservationUserId = typeof localReservation.userId === 'string' 
      ? localReservation.userId 
      : localReservation.userId._id;
    
    const participantId = getParticipantId(participant);
    
    return reservationUserId === participantId;
  };
          ...
          <div className="bg-white rounded-xl shadow-sm p-3 sm:p-4 lg:p-6 border border-gray-100">
            ...
            <div className="sm:hidden space-y-3">
              {localReservation.participants.map((p, i) => {
                const isMain = isMainReserverParticipant(p);
                return (
                  <div key={i} className="p-3 bg-white rounded-lg border border-gray-200 shadow-sm">
                    <div className="space-y-2">
                      <div className="flex justify-between items-start">
                        ...
                        {/* Remove button - hidden for main reserver */}
                        {isMainReserver && ["Pending", "Approved"].includes(localReservation.status) && !isMain && (
                          <button
                  ...
                  {localReservation.participants.map((p, i) => {
                    ...
                    return (
                      <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
                        ...
                        {/* Action column - hidden for main reserver */}
                        {isMainReserver && ["Pending", "Approved"].includes(localReservation.status) && (
                ...
                <div className="space-y-2 sm:space-y-3 max-h-60 sm:max-h-80 overflow-y-auto pr-1 sm:pr-2">
                  {localReservation.participants.length > 0 ? (
                    localReservation.participants.map((participant, index) => {
                      const isMain = isMainReserverParticipant(participant);
                      return (
