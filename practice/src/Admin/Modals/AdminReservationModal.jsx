...
const AdminReservationModal = ({ 
  ...
  const handleUserClick = async (participant) => {
    // Check if participant has a valid ID (24 characters for MongoDB ObjectId)
    if (participant._id && participant._id.length === 24) {
      try {
        setUserLoading(true);
        setUserError("");
      ...
      // If no valid ID, just show the participant data from the reservation
      setSelectedUser(participant);
      setShowUserModal(true);
    }
  };
