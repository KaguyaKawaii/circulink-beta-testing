...
function StaffAnalytics({ setView, staff }) {
  ...
  const fetchReservationAnalytics = useCallback(async () => {
    setLoading(true);
    try {
      let url = `/analytics/staff/reservations?range=${dateRange}`;
      
      if (dateRange === "custom" && customStartDate && customEndDate) {
        url = `/analytics/staff/reservations?startDate=${customStartDate}&endDate=${customEndDate}`;
      }
  ...
  const exportToCSV = () => {
    try {
      ...
      addRow(['Generated by Staff Analytics System']);
      ...
      link.setAttribute('download', `Staff_Reservation_Analytics_${dateRange}_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className="mb-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-xs px-3 py-1.5 bg-blue-50 text-blue-600 rounded-full font-medium">Staff View</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard 
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            {loading ? (
              ...
              reservationData.popularRooms && reservationData.popularRooms.length > 0 ? (
                <div className="space-y-4">
                  {reservationData.popularRooms.slice(0, 5).map((room, idx) => (
                    <ProgressBar 
                      key={idx}
                      label={room.name} 
                      value={room.bookings} 
                ...
                <p className="text-gray-400 text-center py-8">No popular rooms data available</p>
              )
            )}
          </div>
        </div>
