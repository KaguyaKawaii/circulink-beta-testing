...
export const markMessagesAsRead = async (req, res) => {
  try {
    ...
    // FIXED: Handle staff marking messages as read from a specific user
    if (staffId && targetUserId) {
      console.log(`📋 Staff ${staffId} marking messages from user ${targetUserId} as read`);
      
      result = await messageService.markMessagesAsReadFromUser(staffId, targetUserId);
      
      // Update staff's unread counts
      const staffUnreadCount = await messageService.getStaffTotalUnreadCount(staffId);
      io.to(staffId).emit("unreadCountUpdate", { userId: staffId, count: staffUnreadCount });
      
      // Emit specific conversation update for immediate UI refresh
      ...
      // 🆕 ADD THIS: Emit conversation read event for real-time updates
    ...
    // Handle admin conversation marking
    else if (conversationId === "admin") {
      console.log(`📋 Marking admin messages as read for ${actualUserId}`);
      
      result = await messageService.markMessagesAsRead(actualUserId, "admin", messageIds);
