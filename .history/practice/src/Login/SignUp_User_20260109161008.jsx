...
function SignUp_User({ onSwitchToLogin }) {
  ...
  const handleChange = (e) => {
    ...
    if (name === "department") {
      ...
      setShowCourseDropdown(false);
      setShowDepartmentDropdown(false);
      // Show course dropdown immediately after selecting department if role is Student
      if (formData.role === "Student" && value && value !== "Other" && courseOptions[value]) {
        setTimeout(() => {
  ...
  const handleDepartmentSelect = (dept) => {
    ...
    // Show course dropdown immediately after selecting department if role is Student
    if (formData.role === "Student" && dept && dept !== "Other" && courseOptions[dept]) {
      setTimeout(() => {
        setShowCourseDropdown(true);
      }, 100);
    }
  ...
  const handleCourseSelect = (course) => {
    ...
    // Show year level dropdown immediately after selecting course if role is Student
    if (formData.role === "Student" && course) {
      setTimeout(() => {
        setShowYearLevelDropdown(true);
      }, 100);
    }
  };
