...
function SignUp_User({ onSwitchToLogin }) {
  ...
  const isStepComplete = (step) => {
    switch (step) {
      case 1:
        return formData.name && formData.email && formData.id_number && formData.role;
      case 2:
        if (formData.role === "Student") {
                            ...
                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                              {roleOptions.map((role) => (
                                <button
                                  ...
                                  className="w-full px-4 py-3 text-left hover:bg-red-50 hover:text-red-700 transition-colors duration-200 border-b border-gray-100 last:border-b-0 flex items-center gap-2"
                                >
