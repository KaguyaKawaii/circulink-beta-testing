...
import { availableRoomImages } from "./roomImages";
...
function AdminDashboard({ setView }) {
  ...
  // Enhanced room availability calculation
  useEffect(() => {
    if (!reservations.length || !rooms.length) return;

    const availability = {};
    
    // Get all unique rooms from room images configuration
    const allRooms = [...new Set(availableRoomImages.map(img => img.name))];
    ...
    reservations.forEach(reservation => {
      ...
      if (reservationDate >= selectedDateStart && reservationDate <= selectedDateEnd) {
        // Try to find matching room name
        ...
        const matchingRoom = allRooms.find(room => 
          room.toLowerCase().includes(roomName.toLowerCase()) || 
          roomName.toLowerCase().includes(room.toLowerCase())
        );

        if (matchingRoom) {
          if (!availability[matchingRoom]) {
            availability[matchingRoom] = [];
          }
          
          // Avoid duplicates by checking if this reservation already exists
          const existingReservation = availability[matchingRoom].find(
            item => item.time === reservationDate.toLocaleTimeString('en-US', { 
              hour: '2-digit', 
              minute: '2-digit' 
            }) && item.user === (reservation.userId?.name || reservation.userName || 'Unknown User')
          );

          if (!existingReservation) {
            availability[matchingRoom].push({
  ...
  // Get floor information from room images
  const getFloorForRoom = (roomName) => {
    ...
    if (roomImage) {
      if (roomImage.name.includes("1st Floor") || roomImage.category === "Discussion" || roomImage.category === "Graduate") {
        return "1st Floor";
      } else if (roomImage.name.includes("2nd Floor")) {
        return "2nd Floor";
      } else if (roomImage.name.includes("Ground Floor")) {
        return "Ground Floor";
      } else if (roomImage.name.includes("Fifth Floor")) {
        return "5th Floor";
      } else {
        return roomImage.category === "Floor" ? roomImage.name : "Unknown Floor";
      }
    }
    
    // Fallback logic based on room name
    if (roomName.toLowerCase().includes("discussion") || roomName.toLowerCase().includes("graduate")) {
      return "1st Floor";
    } else if (roomName.toLowerCase().includes("2nd") || roomName.toLowerCase().includes("faculty")) {
      return "2nd Floor";
    } else if (roomName.toLowerCase().includes("ground")) {
      return "Ground Floor";
    } else if (roomName.toLowerCase().includes("fifth") || roomName.toLowerCase().includes("5th")) {
      return "5th Floor";
    } else {
  ...
  // Get all unique rooms for display
  const getAllDisplayRooms = () => {
    const roomNames = [...new Set(availableRoomImages.map(img => img.name))];
    
    return roomNames.map(roomName => ({
      name: roomName,
      floor: getFloorForRoom(roomName),
      category: availableRoomImages.find(img => img.name === roomName)?.category || 'General'
    }));
  };
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {getAllDisplayRooms().map((room) => {
              const availability = getAvailabilityStatus(room.name);
              const bookings = getRoomBookings(room.name);
              
              return (
                <div key={room.name} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all duration-300">
                  {/* Room Header */}
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(availability.status)}`}>
                        ...
                        {bookings.map((booking, index) => (
                          <div key={index} className="flex items-center justify-between text-sm p-2 bg-gray-50 rounded-lg">
                            <div>
                              ...
                              <span className="text-gray-500 ml-2">• {booking.user}</span>
                            </div>
                  ...
                  {bookings.length === 0 && (
                    <div className="mt-4 text-center py-3 bg-green-50 rounded-lg border border-green-200">
                      <Check size={16} className="mx-auto text-green-600 mb-1" />
                      <p className="text-sm text-green-700 font-medium">No bookings</p>
