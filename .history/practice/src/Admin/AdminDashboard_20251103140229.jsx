...
function AdminDashboard({ setView }) {
  ...
  // Define room structure based on your example
  const roomStructure = {
    "Ground Floor": [
      "Discussion Room 1",
      "Discussion Room 2", 
  ...
  useEffect(() => {
    ...
    reservations.forEach(reservation => {
      ...
      if (reservationDate >= selectedDateStart && reservationDate <= selectedDateEnd) {
        ...
        // Find matching room name
        let matchedRoom = null;
        Object.entries(roomStructure).forEach(([floor, floorRooms]) => {
          const foundRoom = floorRooms.find(room => 
            room.toLowerCase() === reservationRoomName.toLowerCase() ||
            reservationRoomName.toLowerCase().includes(room.toLowerCase()) ||
            room.toLowerCase().includes(reservationRoomName.toLowerCase())
          );
          if (foundRoom) {
