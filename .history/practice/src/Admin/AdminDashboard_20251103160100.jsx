      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              {newsList.length > 0 ? (
                <div className="space-y-4">
                  {newsList.slice(0, 3).map((news) => (
                    <div key={news._id} className="p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors duration-200">
          ...
          <div className="space-y-6">
            {/* Calendar */}
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
              <h2 className="text-xl font-semibold text-gray-900 mb-6">Room Availability</h2>
              <div className="mb-4">
                <Calendar
              ...
              {/* Quick Room Status */}
              <div className="space-y-3">
                <h3 className="font-semibold text-gray-800 text-sm">Room Status for {selectedDate.toLocaleDateString()}</h3>
                {rooms.slice(0, 4).map((room) => {
                  const status = getAvailabilityStatus(room);
                  return (
                    <div key={room._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex items-center space-x-3">
                        <div className={`w-3 h-3 rounded-full ${
                          status.status === 'available' ? 'bg-green-500' :
                          status.status === 'booked' ? 'bg-red-500' : 'bg-amber-500'
                        }`}></div>
                        <span className="text-sm font-medium text-gray-900">{room.room}</span>
                      </div>
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${getStatusColor(status.status)}`}>
                        {status.message}
                      </span>
                    </div>
                  );
                ...
                {rooms.length > 4 && (
                  <button
                    ...
                    className="w-full flex items-center justify-center p-3 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg border border-blue-200 hover:bg-blue-100 transition-colors duration-200"
                  >
