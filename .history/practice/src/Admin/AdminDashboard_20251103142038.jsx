...
function AdminDashboard({ setView }) {
  ...
  // Define room structure with distinct 4th and 5th floor rooms
  const roomStructure = {
    "Ground Floor": [
      "Discussion Room 1",
      "Discussion Room 2", 
    ...
    "4th Floor": [
      "4th Floor Collaboration Room",
      "4th Floor Faculty Room"
    ],
    "5th Floor": [
      "5th Floor Collaboration Room", 
      "5th Floor Faculty Room"
    ]
  };
  ...
  // Enhanced room availability calculation with proper floor distinction
  useEffect(() => {
    if (!reservations.length) return;

    const availability = {};
    
    // Initialize all rooms from room structure
    Object.entries(roomStructure).forEach(([floor, floorRooms]) => {
      floorRooms.forEach(roomName => {
    ...
    reservations.forEach(reservation => {
      ...
      if (reservationDate >= selectedDateStart && reservationDate <= selectedDateEnd) {
        ...
        // Enhanced room matching with floor distinction
        let matchedRoom = null;
        
        // First try exact match
        Object.entries(roomStructure).forEach(([floor, floorRooms]) => {
          const exactMatch = floorRooms.find(room => 
            room.toLowerCase() === reservationRoomName.toLowerCase()
          );
          if (exactMatch) {
            matchedRoom = exactMatch;
          }
        });

        // If no exact match, try partial match with floor context
        if (!matchedRoom) {
          Object.entries(roomStructure).forEach(([floor, floorRooms]) => {
            floorRooms.forEach(room => {
              // Check if reservation room name contains floor-specific room name
              const roomNameWithoutFloor = room.replace(/^\d+(st|nd|rd|th) Floor /i, '');
              const reservationContainsRoom = reservationRoomName.toLowerCase().includes(roomNameWithoutFloor.toLowerCase());
              const roomContainsReservation = room.toLowerCase().includes(reservationRoomName.toLowerCase());
              
              if (reservationContainsRoom || roomContainsReservation) {
                // Additional check to ensure floor consistency
                const reservationHasFloor = reservationRoomName.toLowerCase().includes('4th') || 
                                          reservationRoomName.toLowerCase().includes('5th') ||
                                          reservationRoomName.toLowerCase().includes('fourth') ||
                                          reservationRoomName.toLowerCase().includes('fifth');
                
                const roomHasFloor = room.toLowerCase().includes('4th') || 
                                   room.toLowerCase().includes('5th') ||
                                   room.toLowerCase().includes('fourth') ||
                                   room.toLowerCase().includes('fifth');
                
                // If both have floor info, they must match
                if (reservationHasFloor && roomHasFloor) {
                  const reservationFloor = reservationRoomName.toLowerCase().includes('4th') || reservationRoomName.toLowerCase().includes('fourth') ? '4th' : '5th';
                  const roomFloor = room.toLowerCase().includes('4th') || room.toLowerCase().includes('fourth') ? '4th' : '5th';
                  
                  if (reservationFloor === roomFloor) {
                    matchedRoom = room;
                  }
                } else if (!reservationHasFloor && !roomHasFloor) {
                  // If neither has floor info, use the room
                  matchedRoom = room;
                }
              }
            });
        ...
        if (matchedRoom && availability[matchedRoom]) {
          // Check for duplicate reservations
          const isDuplicate = availability[matchedRoom].some(
            existing => 
  ...
  // Get room status based on bookings
  ...
  // Helper function to get display name without floor prefix
  const getDisplayRoomName = (roomName) => {
    return roomName.replace(/^\d+(st|nd|rd|th) Floor /i, '');
  };
  ...
  return (
    <main className="ml-[250px] w-[calc(100%-250px)] min-h-screen bg-gray-50">
      ...
      <div className="p-6 space-y-6">
        ...
        {/* NEW: Room Availability Section (Similar to RoomAvailabilityModal) */}
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-900 flex items-center">
          ...
          {/* Room Availability by Floor */}
          <div className="space-y-4">
            {Object.entries(roomStructure).map(([floorName, floorRooms]) => (
              <div key={floorName} className="border border-gray-200 rounded-lg overflow-hidden">
                {/* Floor Header */}
                <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                  <h3 className="font-semibold text-gray-800 text-base">{floorName}</h3>
                </div>
                
                {/* Room List */}
                <div className="divide-y divide-gray-100">
                  {floorRooms.map((roomName) => {
                    const { status, color } = getRoomStatus(roomName);
                    const bookings = getRoomBookings(roomName);

                    return (
                      <div key={roomName} className="p-4">
                          ...
                          <div className="flex items-center gap-3 min-w-0 flex-1">
                            ...
                            <div>
                              <p className="font-medium text-gray-900 text-sm">
                                {getDisplayRoomName(roomName)}
                              </p>
                              <p className="text-xs text-gray-500 mt-0.5">{floorName}</p>
                            </div>
                          </div>
                          ...
                          <div className="text-right flex-shrink-0">
                            {status === 'occupied' ? (
                              <div className="space-y-2">
                                <span className="text-xs text-red-600 bg-red-100 px-2 py-1 rounded">Occupied</span>
                                {bookings.slice(0, 2).map((booking, i) => (
                                  <div key={i} className="text-xs text-gray-600 bg-white px-2 py-1 rounded border border-gray-200">
                                    <div className="font-medium">{booking.time}</div>
                                    <div className="text-gray-500 text-xs">{booking.user}</div>
                                ...
                                {bookings.length > 2 && (
                                  <span className="text-xs text-gray-500">+{bookings.length - 2} more bookings</span>
                                )}
                              </div>
                            ) : status === 'pending' ? (
                              <div className="space-y-2">
                                <span className="text-xs text-amber-600 bg-amber-100 px-2 py-1 rounded">Pending Approval</span>
                                {bookings.slice(0, 1).map((booking, i) => (
                                  <div key={i} className="text-xs text-gray-600 bg-white px-2 py-1 rounded border border-gray-200">
                                    <div className="font-medium">{booking.time}</div>
                                    <div className="text-gray-500 text-xs">{booking.user}</div>
                                ...
                                {bookings.length > 1 && (
                                  <span className="text-xs text-gray-500">+{bookings.length - 1} more pending</span>
                                )}
                              </div>
                            ) : (
