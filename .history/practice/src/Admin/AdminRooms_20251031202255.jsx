...
function AdminRooms({ setView }) {
  ...
  // Room image mapping function (same as ReserveRoom.jsx)
  const getRoomImage = (room) => {
    // If room has a directly assigned image, use it
    if (room.image?.url) {
      return room.image.url;
    }

    // Fallback: Direct mapping of room names to image IDs
  ...
  // Default image suggestion for new rooms
  const getDefaultImageForFloor = (floor) => {
    const floorImageMap = {
      "Ground Floor": getRoomImageById("ground_floor"),
      "2nd Floor": getRoomImageById("second_floor_1"),
      "4th Floor": getRoomImageById("fifth_floor"),
      "5th Floor": getRoomImageById("fifth_floor"),
    };
    
    return floorImageMap[floor] || getRoomImageById("ground_floor");
  };
  ...
  // Auto-suggest image when floor is selected
  const handleFloorChange = (selectedFloor) => {
    setFloor(selectedFloor);
    // Auto-suggest a default image if no image is selected
    if (!roomImage) {
      const defaultImage = getDefaultImageForFloor(selectedFloor);
      if (defaultImage) {
        setRoomImage(defaultImage);
      }
    }
  };
...
function FloorFilter({ floors, selectedFloor, setSelectedFloor, roomsByFloor, getFloorImage }) {
  return (
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {rooms.map((room) => (
            ...
            >
              {/* Room Image Preview with fallback */}
                    ...
                    e.target.parentElement.innerHTML = `
                      <div class="w-full h-full flex items-center justify-center">
                        <div class="text-center">
                          <Building size={24} class="text-gray-400 mx-auto mb-1" />
                          <p class="text-xs text-gray-500 font-medium">${room.room}</p>
                          <p class="text-xs text-gray-400">${room.floor}</p>
                        </div>
                      </div>
                    `;
                  }}
