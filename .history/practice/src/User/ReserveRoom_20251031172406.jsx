...
// FIXED: Enhanced room-to-image mapping with better matching
const getRoomImage = (room) => {
  // Enhanced mapping with multiple possible names and fallbacks
  const roomMappings = {
    ...
    // 2nd Floor Discussion Rooms - multiple possible names
    "2nd Floor Discussion Room 1": "2nd_discussion_room_1_2",
    "Discussion Room 1": "2nd_discussion_room_1_2", // fallback
    "2nd Floor Discussion Room 1.1": "2nd_discussion_room_1_1",
    "Discussion Room 1.1": "2nd_discussion_room_1_1", // fallback
    
    "2nd Floor Discussion Room 2": "2nd_discussion_room_2_2", 
    "Discussion Room 2": "2nd_discussion_room_2_2", // fallback
    "2nd Floor Discussion Room 2.1": "2nd_discussion_room_2_1",
    "Discussion Room 2.1": "2nd_discussion_room_2_1", // fallback
    
    // 2nd Floor Faculty Rooms
    "2nd Floor Faculty Room 1.1": "2nd_faculty_room_1_1",
    "Faculty Room 1.1": "2nd_faculty_room_1_1", // fallback
    "2nd Floor Faculty Room 1": "2nd_faculty_room_1_2",
    "Faculty Room 1": "2nd_faculty_room_1_2", // fallback
  ...
  let imageId = roomMappings[room.room];
  
  // If no exact match, try partial matching for 2nd floor rooms
  if (!imageId && room.floor === "2nd Floor") {
    if (room.room.includes("Discussion Room 1")) {
      imageId = room.room.includes("1.1") ? "2nd_discussion_room_1_1" : "2nd_discussion_room_1_2";
    } else if (room.room.includes("Discussion Room 2")) {
      imageId = room.room.includes("2.1") ? "2nd_discussion_room_2_1" : "2nd_discussion_room_2_2";
    } else if (room.room.includes("Faculty Room")) {
      imageId = room.room.includes("1.1") ? "2nd_faculty_room_1_1" : "2nd_faculty_room_1_2";
    }
  }

  console.log('🖼️ Room image lookup:', {
    roomName: room.room,
    floor: room.floor,
    imageId: imageId,
    found: !!imageId
  ...
  // Enhanced fallback based on floor
  console.log('🔄 Using floor fallback for:', room.floor);
  if (room.floor === "Ground Floor") {
    return getRoomImageById("ground_floor")?.url;
  } else if (room.floor === "2nd Floor") {
    // Use 2nd floor images as fallback
