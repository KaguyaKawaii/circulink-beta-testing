...
function Navigation_User({ user: initialUser, setView, currentView, onLogout }) {
  ...
  // FIXED: Improved fetchUnreadCounts with better error handling and debugging
  const fetchUnreadCounts = async () => {
    ...
    try {
      ...
      // FIXED: Use Promise.allSettled to handle individual failures
  ...
  // FIXED: Add manual refresh function for debugging
  const manuallyRefreshCounts = async () => {
  ...
  useEffect(() => {
    window.manuallyRefreshCounts = manuallyRefreshCounts;
    return () => {
      delete window.manuallyRefreshCounts;
    };
  }, []);
  ...
  useEffect(() => {
    if (currentView === "messages") {
      console.log('💬 Switched to messages view, refreshing counts...');
    ...
    <>
      ...
      {process.env.NODE_ENV === 'development' && (
        <button
          ...
          title="Refresh Counts (Debug)"
        >
          <RefreshCw size={16} className={isLoadingCounts ? 'animate-spin' : ''} />
        </button>
      )}
            ...
            <div className="flex flex-col gap-2 flex-grow">
              {navButtons.map((btn) => (
                ...
                >
                  ...
                  {isLoadingCounts && (btn.id === 'notification' || btn.id === 'messages') && (
