...
const getRoomImage = (room) => {
  ...
  const directMappings = {
    ...
    // 2nd Floor Discussion Rooms - FIXED: Match exact room names from your database
    ...
    // 2nd Floor Faculty Rooms - FIXED: Match exact room names from your database
    "2nd Floor Faculty Room 1.1": "2nd_faculty_room_1_1",
    "2nd Floor Faculty Room 1": "2nd_faculty_room_1_2",
    
    // 5th Floor Rooms
    "Faculty Room": "faculty_room",      // 5th floor faculty room
    "Collaboration Room": "collab_room", // 5th floor collaboration room
  };
  ...
  // Debug: Log rooms that don't have direct mappings
  console.log('Room without direct image mapping:', room.room, 'on floor:', room.floor);

  // Fallback based on floor - only use this as last resort
  if (room.floor === "Ground Floor") {
    return getRoomImageById("ground_floor")?.url;
  } else if (room.floor === "2nd Floor") {
    // For 2nd floor, try to find any room image that matches the room name
    const roomImages = availableRoomImages.filter(img => 
      img.name.toLowerCase().includes(room.room.toLowerCase()) ||
      room.room.toLowerCase().includes(img.name.toLowerCase())
    );
    if (roomImages.length > 0) {
      return roomImages[0].url;
    }
