...
// UPDATED: More robust room image mapping with fallbacks
const getRoomImage = (room) => {
  console.log('🔍 Looking for image for room:', room.room, 'on floor:', room.floor);

  // Enhanced mapping with multiple possible name formats
  const directMappings = {
    ...
    // 2nd Floor Discussion Rooms - Multiple possible formats
    ...
    // 2nd Floor Faculty Rooms - Multiple possible formats
  ...
  let imageId = directMappings[room.room];
  
  // If no exact match, try partial matching for 2nd floor
  if (!imageId && room.floor === "2nd Floor") {
    if (room.room.includes("Discussion") && room.room.includes("1.1")) {
      imageId = "2nd_discussion_room_1_1";
    } else if (room.room.includes("Discussion") && room.room.includes("1")) {
      imageId = "2nd_discussion_room_1_2";
    } else if (room.room.includes("Discussion") && room.room.includes("2.1")) {
      imageId = "2nd_discussion_room_2_1";
    } else if (room.room.includes("Discussion") && room.room.includes("2")) {
      imageId = "2nd_discussion_room_2_2";
    } else if (room.room.includes("Faculty") && room.room.includes("1.1")) {
      imageId = "2nd_faculty_room_1_1";
    } else if (room.room.includes("Faculty") && room.room.includes("1")) {
      imageId = "2nd_faculty_room_1_2";
    }
  }

  console.log('🎯 Found image ID:', imageId, 'for room:', room.room);
  ...
  console.log('🔄 Using fallback floor image for:', room.floor);
