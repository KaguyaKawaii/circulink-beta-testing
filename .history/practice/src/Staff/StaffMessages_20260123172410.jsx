...
function StaffMessages({ setView, staff, onLogout }) {
  ...
  const markMessagesAsRead = async (userId) => {
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/messages/mark-read`, {
        staffId: staff._id,
        targetUserId: userId  // Changed from userId to targetUserId
      });
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Handle conversation marked as read
    const handleConversationRead = (data) => {
      console.log("Conversation marked as read:", data);
      
      if (data.staffId === staff._id) {
        // Immediately update the conversation unread count to 0
        setConversations(prev => 
          prev.map(conv => 
            conv._id === data.userId 
              ? { ...conv, unreadCount: 0 } 
    ...
    socket.on("conversationRead", handleConversationRead); // ADD THIS
    ...
    socket.on("messageSent", (msg) => {
      ...
      // When we send a message, mark the conversation as read locally
      if (activeTab === "floor" && selectedUser) {
        setConversations(prev => 
          prev.map(conv => 
            conv._id === selectedUser._id 
    ...
    return () => {
      ...
      socket.off("conversationRead", handleConversationRead); // ADD THIS
  ...
  const fetchUserConversation = async (user) => {
    try {
      ...
      // Immediately mark as read when opening conversation
  ...
  const sendMessage = async () => {
    ...
    if (activeTab === "floor" && selectedUser) {
      ...
      try {
        ...
        // Also call mark as read directly
        await axios.put(`${import.meta.env.VITE_API_URL}/api/messages/mark-read`, {
          staffId: staff._id,
