import React, { useState, useEffect, useRef, useCallback, useMemo } from "react";
import axios from "axios";
import { io } from "socket.io-client";
import StaffNavigation from "./StaffNavigation";

const socket = io(`${import.meta.env.VITE_API_URL}`);

// Constants
const MESSAGE_TYPES = {
  FLOOR: "floor",
  ADMIN: "admin"
};

// Utility Functions
const formatTime = (iso) => {
  return new Date(iso).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
};

const formatDate = (iso) => {
  const date = new Date(iso);
  const today = new Date();
  const yesterday = new Date(today);
  yesterday.setDate(yesterday.getDate() - 1);
  
  if (date.toDateString() === today.toDateString()) return "Today";
  if (date.toDateString() === yesterday.toDateString()) return "Yesterday";
  return date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
};

// Extracted UI Components
const ConversationItem = ({ conv, isSelected, onClick, formatTime }) => (
  <button
    onClick={onClick}
    className={`w-full text-left p-4 rounded-xl transition-all duration-200 ${
      isSelected 
        ? 'bg-gradient-to-r from-red-50 to-orange-50 border-2 border-red-200 shadow-lg' 
        : 'bg-white hover:bg-gray-50 border border-gray-200 hover:border-gray-300 hover:shadow-md'
    }`}
  >
    <div className="flex items-center justify-between">
      <div className="flex items-center space-x-3 flex-1 min-w-0">
        {conv.unreadCount > 0 && (
          <div className="bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center shrink-0 shadow-sm">
            {conv.unreadCount > 9 ? "9+" : conv.unreadCount}
          </div>
        )}
        <div className="w-10 h-10 bg-gradient-to-r from-red-500 to-orange-500 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-sm">
          {conv.name.charAt(0).toUpperCase()}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <span className="font-semibold text-gray-800 text-sm truncate">
              {conv.name}
            </span>
            <span className="text-xs text-gray-500 shrink-0 ml-2">
              {conv.latestMessageAt ? formatTime(conv.latestMessageAt) : ''}
            </span>
          </div>
          <div className="text-xs text-gray-600 truncate mt-1">
            {conv.latestMessage || "No messages yet"}
          </div>
        </div>
      </div>
    </div>
  </button>
);

const MessageBubble = ({ message, isOwn, isUnread, activeTab, staff, formatTime }) => {
  const senderName = isOwn 
    ? activeTab === MESSAGE_TYPES.FLOOR ? `${staff.floor} Staff` : staff.name
    : activeTab === MESSAGE_TYPES.FLOOR ? message.senderName : 'Administration';

  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`max-w-[85%] lg:max-w-[70%] rounded-2xl p-4 shadow-sm transition-all duration-300 hover:shadow-md relative ${
        isOwn 
          ? activeTab === MESSAGE_TYPES.FLOOR 
            ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-br-none' 
            : 'bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-br-none'
          : 'bg-white border border-gray-200 rounded-bl-none shadow-sm'
      }`}>
        {/* NEW BADGE for unread messages */}
        {isUnread && !isOwn && (
          <div className="absolute -top-2 -left-2 z-10">
            <span className="bg-green-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg flex items-center animate-pulse">
              <svg className="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z" clipRule="evenodd" />
              </svg>
              NEW
            </span>
          </div>
        )}
        
        <div className="text-sm font-semibold mb-2">
          {senderName}
        </div>
        <div className="text-sm leading-relaxed break-words whitespace-pre-wrap">{message.content}</div>
        <div className={`text-xs mt-2 text-right flex items-center justify-end ${
          isOwn 
            ? activeTab === MESSAGE_TYPES.FLOOR ? 'text-red-100' : 'text-blue-100'
            : 'text-gray-500'
        }`}>
          {formatTime(message.createdAt)}
          {isOwn && message.status === "sending" && (
            <div className="text-xs opacity-80 flex items-center ml-2">
              <svg className="w-3 h-3 mr-1 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 2v4m0 12v4m8-10h-4M6 12H2" />
              </svg>
              Sending...
            </div>
          )}
          {isOwn && message.status === "failed" && (
            <div className="text-xs opacity-80 text-red-200 flex items-center ml-2">
              <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              Failed
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

const DateSeparator = ({ date }) => {
  return (
    <div className="flex items-center justify-center my-6">
      <div className="bg-gray-200 text-gray-600 text-xs px-4 py-1.5 rounded-full">
        {date}
      </div>
    </div>
  );
};

const LoadingSkeleton = () => (
  <div className="flex justify-center items-center h-full">
    <div className="text-center">
      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-red-600 mx-auto mb-4"></div>
      <p className="text-gray-600">Loading messages...</p>
    </div>
  </div>
);

const EmptyState = ({ activeTab, selectedUser }) => (
  <div className="flex justify-center items-center h-full min-h-[60vh]">
    <div className="text-center text-gray-500 max-w-xs mx-auto">
      <svg className="w-16 h-16 mx-auto mb-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
      </svg>
      <p className="font-medium text-gray-700 mb-2">No messages yet</p>
      <p className="text-sm text-gray-600">
        {activeTab === MESSAGE_TYPES.FLOOR && selectedUser 
          ? `Start a conversation with ${selectedUser.name}` 
          : "Contact administration for support"}
      </p>
    </div>
  </div>
);

function StaffMessages({ setView, staff, onLogout }) {
  const [conversations, setConversations] = useState([]);
  const [selectedUser, setSelectedUser] = useState(null);
  const [messages, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");
  const [loading, setLoading] = useState(false);
  const [activeTab, setActiveTab] = useState(MESSAGE_TYPES.FLOOR);
  const [searchTerm, setSearchTerm] = useState("");
  const [totalUnread, setTotalUnread] = useState(0);
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);
  const [isChatFullscreen, setIsChatFullscreen] = useState(false);
  const [unreadMessageIds, setUnreadMessageIds] = useState(new Set());
  
  const chatContainerRef = useRef(null);
  const textareaRef = useRef(null);
  const messagesEndRef = useRef(null);

  // Check screen size
  useEffect(() => {
    const checkMobile = () => {
      setIsMobile(window.innerWidth < 768);
      if (window.innerWidth < 768) {
        setIsSidebarOpen(false);
      } else {
        setIsSidebarOpen(true);
      }
    };

    checkMobile();
    window.addEventListener("resize", checkMobile);
    return () => window.removeEventListener("resize", checkMobile);
  }, []);

  // Toggle chat fullscreen on mobile
  useEffect(() => {
    if (isMobile && selectedUser) {
      setIsChatFullscreen(true);
    } else {
      setIsChatFullscreen(false);
    }
  }, [isMobile, selectedUser]);

  // Auto-scroll to bottom
  const scrollToBottom = useCallback(() => {
    if (chatContainerRef.current) {
      chatContainerRef.current.scrollTop = chatContainerRef.current.scrollHeight;
    }
  }, []);

  useEffect(() => {
    scrollToBottom();
  }, [messages, scrollToBottom]);

  // Auto-resize textarea
  useEffect(() => {
    if (textareaRef.current) {
      textareaRef.current.style.height = "auto";
      textareaRef.current.style.height = `${Math.min(textareaRef.current.scrollHeight, 100)}px`;
    }
  }, [newMessage]);

  // API Functions
  const fetchTotalUnreadCount = useCallback(async () => {
    if (!staff?._id) return;
    
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/messages/staff-total-unread/${staff._id}`
      );
      setTotalUnread(data.count || 0);
    } catch (err) {
      console.error("Failed to fetch total unread count:", err);
      setTotalUnread(0);
    }
  }, [staff]);

  const fetchConversations = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/messages/floor-users/${staff.floor}`
      );
      
      const conversationsWithUnread = await Promise.all(
        data.map(async (conv) => {
          try {
            const { data: unreadData } = await axios.get(
              `${import.meta.env.VITE_API_URL}/api/messages/unread-count-by-user/${staff._id}/${conv._id}`
            );
            return { ...conv, unreadCount: unreadData.count || 0 };
          } catch (err) {
            console.error(`Failed to fetch unread count for ${conv._id}:`, err);
            return { ...conv, unreadCount: 0 };
          }
        })
      );
      
      setConversations(conversationsWithUnread);
    } catch (err) {
      console.error("Failed to fetch conversations:", err);
    }
  }, [staff]);

  const fetchUnreadMessages = useCallback(async () => {
    if (!staff?._id) return;
    
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/messages/unread-messages/${staff._id}`
      );
      
      if (response.data.success) {
        const unreadIds = response.data.unreadMessages.map(msg => msg._id);
        setUnreadMessageIds(new Set(unreadIds));
      }
    } catch (error) {
      console.error("Failed to fetch unread messages:", error);
    }
  }, [staff]);

  const markMessagesAsRead = useCallback(async (userId) => {
    if (!staff?._id || !userId) return;
    
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/messages/mark-read`, {
        staffId: staff._id,
        targetUserId: userId
      });
      
      // Clear unread message IDs for this conversation
      setUnreadMessageIds(prev => {
        const newSet = new Set(prev);
        // Remove all messages from this user from unread set
        messages.forEach(msg => {
          if (msg.sender === userId) {
            newSet.delete(msg._id);
          }
        });
        return newSet;
      });
      
      // Update conversations list
      setConversations(prev => 
        prev.map(conv => 
          conv._id === userId 
            ? { ...conv, unreadCount: 0 } 
            : conv
        )
      );
      
      // Update selected user if applicable
      if (selectedUser && selectedUser._id === userId) {
        setSelectedUser(prev => prev ? { ...prev, unreadCount: 0 } : null);
      }
      
      // Refresh counts
      fetchTotalUnreadCount();
      
    } catch (err) {
      console.error("Failed to mark messages as read:", err);
    }
  }, [staff, selectedUser, messages, fetchTotalUnreadCount]);

  const markAdminMessagesAsRead = useCallback(async () => {
    if (!staff?._id) return;
    
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/messages/mark-read`, {
        staffId: staff._id,
        conversationId: "admin"
      });
      
      // Clear all admin unread messages
      setUnreadMessageIds(prev => {
        const newSet = new Set(prev);
        messages.forEach(msg => {
          if (msg.sender === "admin") {
            newSet.delete(msg._id);
          }
        });
        return newSet;
      });
      
      fetchTotalUnreadCount();
      
    } catch (err) {
      console.error("Failed to mark admin messages as read:", err);
    }
  }, [staff, messages, fetchTotalUnreadCount]);

  const isMessageUnread = useCallback((messageId) => {
    return unreadMessageIds.has(messageId);
  }, [unreadMessageIds]);

  const groupMessagesByDate = useCallback(() => {
    const groups = {};
    messages.forEach(msg => {
      const date = formatDate(msg.createdAt);
      if (!groups[date]) groups[date] = [];
      
      const messageExists = groups[date].some(m => m._id === msg._id);
      if (!messageExists) {
        groups[date].push(msg);
      }
    });
    return groups;
  }, [messages]);

  const messageGroups = useMemo(() => groupMessagesByDate(), [groupMessagesByDate]);

  // Socket and Data Fetching
  useEffect(() => {
    if (!staff?._id) return;

    let isMounted = true;

    socket.emit("join", { userId: staff._id });
    socket.emit("join", { userId: staff.floor });

    const initializeData = async () => {
      if (!isMounted) return;
      
      await fetchTotalUnreadCount();
      await fetchUnreadMessages();
      
      if (activeTab === MESSAGE_TYPES.FLOOR) {
        await fetchConversations();
      } else {
        await fetchAdminConversation();
      }
    };

    initializeData();

    const handleNewMessage = (msg) => {
      if (!isMounted) return;
      
      console.log("📨 New message received in StaffMessages:", msg);
      
      // Refresh unread counts
      fetchTotalUnreadCount();
      
      if (activeTab === MESSAGE_TYPES.FLOOR) {
        fetchConversations();
      }
      
      if (activeTab === MESSAGE_TYPES.ADMIN) {
        fetchAdminConversation();
      }
      
      // Handle message addition to current chat
      if (activeTab === MESSAGE_TYPES.FLOOR && selectedUser) {
        const isRelevantMessage = 
          (msg.sender === selectedUser._id && msg.receiver === staff._id) ||
          (msg.sender === staff._id && msg.receiver === selectedUser._id) ||
          (msg.floor === staff.floor && msg.receiver === selectedUser._id) ||
          (msg.sender === selectedUser._id && msg.floor === staff.floor) ||
          (msg.receiver === staff.floor && msg.sender === selectedUser._id);

        if (isRelevantMessage) {
          setMessages(prev => {
            const filtered = prev.filter(m => 
              !(m.status === "sending" && m.content === msg.content && m.sender === msg.sender)
            );
            
            const exists = filtered.some(m => m._id === msg._id);
            if (!exists) {
              return [...filtered, msg];
            }
            return filtered;
          });
          
          // If message is from user, mark it as read
          if (msg.sender !== staff._id) {
            setUnreadMessageIds(prev => {
              const newSet = new Set(prev);
              newSet.add(msg._id);
              return newSet;
            });
            
            // Also mark conversation as read via API
            markMessagesAsRead(selectedUser._id);
          }
        }
      }
      
      if (activeTab === MESSAGE_TYPES.ADMIN) {
        const isRelevantAdminMessage = 
          (msg.sender === "admin" && msg.receiver === staff._id) ||
          (msg.sender === staff._id && msg.receiver === "admin");

        if (isRelevantAdminMessage) {
          setMessages(prev => {
            const messageExists = prev.some(m => m._id === msg._id);
            if (!messageExists) {
              return [...prev, msg];
            }
            return prev;
          });
          
          if (msg.sender === "admin") {
            setUnreadMessageIds(prev => {
              const newSet = new Set(prev);
              newSet.add(msg._id);
              return newSet;
            });
            
            markAdminMessagesAsRead();
          }
        }
      }
    };

    const handleUnreadCountUpdate = (data) => {
      if (isMounted && data.userId === staff._id) {
        fetchTotalUnreadCount();
        if (activeTab === MESSAGE_TYPES.FLOOR) {
          fetchConversations();
        }
      }
    };

    const handleConversationUnreadUpdate = (data) => {
      if (isMounted && data.staffId === staff._id) {
        setConversations(prev => 
          prev.map(conv => 
            conv._id === data.userId 
              ? { ...conv, unreadCount: data.count || 0 } 
              : conv
          )
        );
        
        if (selectedUser && selectedUser._id === data.userId) {
          setSelectedUser(prev => prev ? { ...prev, unreadCount: data.count || 0 } : null);
        }
        
        fetchTotalUnreadCount();
      }
    };

    const handleConversationRead = (data) => {
      if (isMounted && data.staffId === staff._id) {
        // Clear unread counts for this conversation
        setConversations(prev => 
          prev.map(conv => 
            conv._id === data.userId 
              ? { ...conv, unreadCount: 0 } 
              : conv
          )
        );
        
        if (selectedUser && selectedUser._id === data.userId) {
          setSelectedUser(prev => prev ? { ...prev, unreadCount: 0 } : null);
        }
        
        // Clear unread message IDs for this conversation
        setUnreadMessageIds(prev => {
          const newSet = new Set(prev);
          messages.forEach(msg => {
            if (msg.sender === data.userId || msg.receiver === data.userId) {
              newSet.delete(msg._id);
            }
          });
          return newSet;
        });
        
        fetchTotalUnreadCount();
      }
    };

    const handleMessageSent = (msg) => {
      if (!isMounted) return;
      
      console.log("✅ Message sent confirmation:", msg);
      setMessages(prev => prev.map(m => 
        m.status === "sending" && m.content === msg.content 
          ? { ...msg, status: "sent" }
          : m
      ));
      
      // When staff sends a message, immediately clear unread badges for that conversation
      if (activeTab === MESSAGE_TYPES.FLOOR && selectedUser) {
        setConversations(prev => 
          prev.map(conv => 
            conv._id === selectedUser._id 
              ? { ...conv, unreadCount: 0 } 
              : conv
          )
        );
        
        setSelectedUser(prev => prev ? { ...prev, unreadCount: 0 } : null);
        
        // Clear unread message IDs for this conversation
        setUnreadMessageIds(prev => {
          const newSet = new Set(prev);
          messages.forEach(msg => {
            if (msg.sender === selectedUser._id || msg.receiver === selectedUser._id) {
              newSet.delete(msg._id);
            }
          });
          return newSet;
        });
        
        fetchTotalUnreadCount();
      }
    };

    socket.on("newMessage", handleNewMessage);
    socket.on("unreadCountUpdate", handleUnreadCountUpdate);
    socket.on("conversationUnreadUpdate", handleConversationUnreadUpdate);
    socket.on("conversationRead", handleConversationRead);
    socket.on("messageSent", handleMessageSent);
    
    return () => {
      isMounted = false;
      socket.off("newMessage", handleNewMessage);
      socket.off("unreadCountUpdate", handleUnreadCountUpdate);
      socket.off("conversationUnreadUpdate", handleConversationUnreadUpdate);
      socket.off("conversationRead", handleConversationRead);
      socket.off("messageSent", handleMessageSent);
    };
  }, [staff, selectedUser, activeTab, fetchTotalUnreadCount, fetchConversations, fetchUnreadMessages, markMessagesAsRead, markAdminMessagesAsRead, messages]);

  const fetchAdminConversation = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/messages/staff-admin-conversation/${staff._id}`
      );
      setMessages(data);
      
      // Mark admin messages as read when opening admin chat
      await markAdminMessagesAsRead();
      
      setTimeout(() => {
        scrollToBottom();
      }, 100);
    } catch (err) {
      console.error("Failed to fetch admin conversation:", err);
    }
  }, [staff, markAdminMessagesAsRead, scrollToBottom]);

  const fetchUserConversation = useCallback(async (user) => {
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/messages/staff-user-conversation/${staff._id}/${user._id}`
      );
      setMessages(data);
      
      // Mark messages as read when opening conversation
      await markMessagesAsRead(user._id);
      
      setTimeout(() => {
        scrollToBottom();
      }, 100);
    } catch (err) {
      console.error("Failed to fetch user conversation:", err);
    }
  }, [staff, markMessagesAsRead, scrollToBottom]);

  const selectUser = useCallback(async (user) => {
    setSelectedUser(user);
    setLoading(true);
    
    if (isMobile) {
      setIsSidebarOpen(false);
      setIsChatFullscreen(true);
    }
    
    try {
      await fetchUserConversation(user);
    } catch (err) {
      console.error("Failed to fetch messages:", err);
    } finally {
      setLoading(false);
    }
  }, [isMobile, fetchUserConversation]);

  const handleTabChange = useCallback((tab) => {
    setActiveTab(tab);
    setSelectedUser(null);
    setMessages([]);
    setNewMessage("");
    
    if (tab === MESSAGE_TYPES.ADMIN) {
      fetchAdminConversation();
      if (isMobile) {
        setIsSidebarOpen(false);
        setIsChatFullscreen(true);
      }
    } else {
      fetchConversations();
    }
  }, [isMobile, fetchAdminConversation, fetchConversations]);

  const handleKeyPress = useCallback((e) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      sendMessage();
    }
  }, []);

  const sendMessage = useCallback(async () => {
    if (!newMessage.trim()) return;

    let tempMsg;
    
    if (activeTab === MESSAGE_TYPES.FLOOR && selectedUser) {
      tempMsg = {
        _id: "temp-" + Date.now(),
        sender: staff._id,
        receiver: selectedUser._id,
        content: newMessage,
        createdAt: new Date().toISOString(),
        status: "sending",
        senderName: `${staff.floor} Staff`,
        displayName: `${staff.floor} Staff`,
        floor: staff.floor
      };

      setMessages(prev => [...prev, tempMsg]);
      setNewMessage("");

      try {
        await axios.post(`${import.meta.env.VITE_API_URL}/api/messages/staff-reply`, {
          staffId: staff._id,
          userId: selectedUser._id,
          content: newMessage,
          floor: staff.floor
        });
        
        // Immediately clear the unread badge when staff sends a message
        setConversations(prev => 
          prev.map(conv => 
            conv._id === selectedUser._id 
              ? { ...conv, unreadCount: 0 } 
              : conv
          )
        );
        
        setSelectedUser(prev => prev ? { ...prev, unreadCount: 0 } : null);
        
        // Clear unread message IDs for this conversation
        setUnreadMessageIds(prev => {
          const newSet = new Set(prev);
          messages.forEach(msg => {
            if (msg.sender === selectedUser._id || msg.receiver === selectedUser._id) {
              newSet.delete(msg._id);
            }
          });
          return newSet;
        });
        
        fetchTotalUnreadCount();
        
      } catch (err) {
        console.error("Failed to send message:", err);
        setMessages(prev => prev.map(msg => 
          msg._id === tempMsg._id ? { ...msg, status: "failed" } : msg
        ));
      }
    } else if (activeTab === MESSAGE_TYPES.ADMIN) {
      tempMsg = {
        _id: "temp-" + Date.now(),
        sender: staff._id,
        receiver: "admin",
        content: newMessage,
        createdAt: new Date().toISOString(),
        status: "sending",
        senderName: staff.name,
        displayName: staff.name
      };

      setMessages(prev => [...prev, tempMsg]);
      setNewMessage("");

      try {
        await axios.post(`${import.meta.env.VITE_API_URL}/api/messages/staff-to-admin`, {
          staffId: staff._id,
          content: newMessage
        });
        
        fetchTotalUnreadCount();
        
        await markAdminMessagesAsRead();
        
      } catch (err) {
        console.error("Failed to send message to admin:", err);
        setMessages(prev => prev.map(msg => 
          msg._id === tempMsg._id ? { ...msg, status: "failed" } : msg
        ));
      }
    }
  }, [activeTab, selectedUser, staff, newMessage, messages, fetchTotalUnreadCount, markAdminMessagesAsRead]);

  const filteredConversations = useMemo(() => 
    conversations.filter(conv =>
      conv.name.toLowerCase().includes(searchTerm.toLowerCase())
    ),
  [conversations, searchTerm]);

  // Function to exit chat view on mobile
  const exitChatView = useCallback(() => {
    setSelectedUser(null);
    setIsChatFullscreen(false);
    if (isMobile) {
      setIsSidebarOpen(true);
    }
  }, [isMobile]);

  // Function to go back to conversations list
  const goBackToConversations = useCallback(() => {
    exitChatView();
  }, [exitChatView]);

  return (
    <>
      {/* Only show navigation when not in fullscreen chat on mobile */}
      {(!isMobile || !isChatFullscreen) && (
        <StaffNavigation setView={setView} currentView="staffMessages" staff={staff} onLogout={onLogout} />
      )}
      
      <div className={`${!isMobile || !isChatFullscreen ? 'md:ml-[250px]' : ''} w-full ${!isMobile || !isChatFullscreen ? 'md:w-[calc(100%-250px)]' : ''} h-screen flex flex-col bg-gray-50`}>
        
        {/* Mobile Header - Only show when not in fullscreen chat */}
        {!isChatFullscreen && (
          <header className="md:hidden bg-white px-4 py-3 border-b border-gray-200 flex items-center justify-between">
            <button
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
              className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200"
              aria-label="Toggle sidebar"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
              </svg>
            </button>
            <div className="flex-1 ml-4">
              <h1 className="text-lg font-bold text-[#CC0000]">Messages</h1>
            </div>
            {totalUnread > 0 && (
              <div className="bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs px-2 py-1 rounded-full font-bold">
                {totalUnread}
              </div>
            )}
          </header>
        )}

        {/* Desktop Header - Always show on desktop */}
        {!isMobile && (
          <header className="bg-white px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-[#CC0000]">Message Center</h1>
                <p className="text-gray-600 text-sm">Communicate with residents and administration</p>
              </div>
              {totalUnread > 0 && (
                <div className="bg-gradient-to-r from-red-500 to-orange-500 text-white text-sm px-3 py-1.5 rounded-full font-bold shadow-sm">
                  {totalUnread} unread
                </div>
              )}
            </div>
          </header>
        )}

        <div className="flex flex-1 overflow-hidden relative">
          {/* Sidebar Overlay for Mobile - Only show when sidebar is open and not in fullscreen chat */}
          {isMobile && isSidebarOpen && !isChatFullscreen && (
            <div 
              className="fixed inset-0 bg-black bg-opacity-50 z-20"
              onClick={() => setIsSidebarOpen(false)}
            />
          )}

          {/* Sidebar - Hide when in fullscreen chat on mobile */}
          {(!isMobile || !isChatFullscreen) && (
            <div className={`
              ${isMobile ? 'fixed inset-y-0 left-0 z-30 transform' : 'relative'} 
              ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
              transition-transform duration-300 ease-in-out
              w-full md:w-80 bg-white border-r border-gray-200 flex flex-col
              md:static md:h-full
            `}>
              <div className="p-4 border-b border-gray-200">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-lg font-bold text-gray-700">Conversations</h2>
                  {isMobile && (
                    <button
                      onClick={() => setIsSidebarOpen(false)}
                      className="p-2 rounded-lg hover:bg-gray-100"
                      aria-label="Close sidebar"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    </button>
                  )}
                </div>
                
                {/* Search */}
                <div className="relative mb-3">
                  <input
                    type="text"
                    placeholder="Search conversations..."
                    className="w-full bg-gray-100 border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-colors"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
                </div>
                
                {/* Tabs */}
                <div className="flex bg-gray-100 rounded-lg p-1">
                  <button
                    onClick={() => handleTabChange(MESSAGE_TYPES.FLOOR)}
                    className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${
                      activeTab === MESSAGE_TYPES.FLOOR 
                        ? "bg-gradient-to-r from-red-500 to-orange-500 text-white shadow-sm" 
                        : "text-gray-600 hover:text-gray-900 hover:bg-gray-200"
                    }`}
                  >
                    Residents
                  </button>
                  <button
                    onClick={() => handleTabChange(MESSAGE_TYPES.ADMIN)}
                    className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${
                      activeTab === MESSAGE_TYPES.ADMIN 
                        ? "bg-gradient-to-r from-blue-500 to-cyan-500 text-white shadow-sm" 
                        : "text-gray-600 hover:text-gray-900 hover:bg-gray-200"
                    }`}
                  >
                    Admin
                  </button>
                </div>
              </div>
              
              {/* Conversations List */}
              <div className="flex-1 overflow-y-auto p-3">
                {activeTab === MESSAGE_TYPES.FLOOR ? (
                  loading ? (
                    <div className="space-y-2">
                      {[1, 2, 3].map(i => (
                        <div key={i} className="w-full p-4 rounded-xl bg-gray-100 animate-pulse">
                          <div className="flex items-center space-x-3">
                            <div className="w-10 h-10 bg-gray-300 rounded-full"></div>
                            <div className="flex-1">
                              <div className="h-4 bg-gray-300 rounded w-24 mb-2"></div>
                              <div className="h-3 bg-gray-300 rounded w-32"></div>
                            </div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : filteredConversations.length === 0 ? (
                    <div className="text-center p-8 text-gray-500">
                      <svg className="w-12 h-12 mx-auto mb-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                      </svg>
                      <p className="font-medium text-gray-700">No conversations</p>
                      <p className="text-sm mt-1 text-gray-500">
                        {searchTerm ? "No matches found" : "Residents will appear here"}
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {filteredConversations.map(conv => (
                        <ConversationItem
                          key={conv._id}
                          conv={conv}
                          isSelected={selectedUser?._id === conv._id}
                          onClick={() => selectUser(conv)}
                          formatTime={formatTime}
                        />
                      ))}
                    </div>
                  )
                ) : (
                  <div className="p-2">
                    <div 
                      onClick={() => handleTabChange(MESSAGE_TYPES.ADMIN)}
                      className={`bg-gradient-to-r from-blue-50 to-cyan-50 border-2 border-blue-200 rounded-xl p-4 cursor-pointer transition-all duration-200 hover:shadow-md ${
                        activeTab === MESSAGE_TYPES.ADMIN && !selectedUser ? 'ring-2 ring-blue-300 ring-opacity-50' : ''
                      }`}
                    >
                      <div className="flex items-center mb-2">
                        <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg flex items-center justify-center text-white mr-3 shrink-0 shadow-sm">
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                          </svg>
                        </div>
                        <div className="flex-1">
                          <span className="font-bold text-blue-800 text-sm block">Admin Support</span>
                          <p className="text-xs text-blue-600">Always available</p>
                        </div>
                      </div>
                      <p className="text-xs text-blue-700 leading-relaxed">
                        Contact administrators for support, questions, or assistance.
                      </p>
                    </div>
                  </div>
                )}
              </div>
            </div>
          )}

          {/* Chat Area */}
          <div className="flex-1 flex flex-col min-w-0">
            {/* Mobile Chat Header - Only show in fullscreen chat */}
            {isMobile && isChatFullscreen && selectedUser && (
              <div className="bg-white px-4 py-3 border-b border-gray-200 flex items-center">
                <button
                  onClick={goBackToConversations}
                  className="p-2 mr-3 rounded-lg hover:bg-gray-100"
                  aria-label="Go back to conversations"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                  </svg>
                </button>
                <div className="flex-1">
                  <h3 className="font-bold text-gray-800 truncate">{selectedUser.name}</h3>
                  <p className="text-gray-600 text-xs">Floor {staff.floor}</p>
                </div>
                {selectedUser.unreadCount > 0 && (
                  <span className="bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs px-2 py-1 rounded font-bold">
                    {selectedUser.unreadCount} unread
                  </span>
                )}
              </div>
            )}

            {activeTab === MESSAGE_TYPES.FLOOR && selectedUser ? (
              <>
                {/* Desktop User Header */}
                {!isMobile && (
                  <div className="bg-white p-4 border-b border-gray-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-gradient-to-r from-red-500 to-orange-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm">
                          {selectedUser.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <h3 className="font-bold text-gray-800">{selectedUser.name}</h3>
                          <p className="text-gray-600 text-sm">Floor {staff.floor}</p>
                        </div>
                      </div>
                      {selectedUser.unreadCount > 0 && (
                        <span className="bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs px-3 py-1.5 rounded-full font-bold shadow-sm">
                          {selectedUser.unreadCount} unread
                        </span>
                      )}
                    </div>
                  </div>
                )}
                
                {/* Messages Area */}
                <div 
                  ref={chatContainerRef}
                  className="flex-1 overflow-y-auto bg-gradient-to-b from-gray-50 to-white"
                >
                  <div className="p-3 md:p-4">
                    {loading ? (
                      <LoadingSkeleton />
                    ) : messages.length === 0 ? (
                      <EmptyState activeTab={activeTab} selectedUser={selectedUser} />
                    ) : (
                      <div className="max-w-3xl mx-auto space-y-4 md:space-y-6">
                        {Object.entries(messageGroups).map(([date, dateMessages]) => (
                          <div key={date} className="space-y-2 md:space-y-3">
                            <DateSeparator date={date} />
                            <div className="space-y-2">
                              {dateMessages.map(msg => (
                                <MessageBubble
                                  key={msg._id}
                                  message={msg}
                                  isOwn={msg.sender === staff._id}
                                  isUnread={isMessageUnread(msg._id)}
                                  activeTab={activeTab}
                                  staff={staff}
                                  formatTime={formatTime}
                                />
                              ))}
                            </div>
                          </div>
                        ))}
                        <div ref={messagesEndRef} />
                      </div>
                    )}
                  </div>
                </div>

                {/* Message Input */}
                <div className="bg-white border-t border-gray-200 p-3 md:p-4">
                  <div className="flex space-x-2 md:space-x-3">
                    <textarea
                      ref={textareaRef}
                      placeholder={`Message ${selectedUser.name}...`}
                      className="flex-1 bg-gray-50 border-2 border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-100 text-sm md:text-base resize-none transition-all duration-200"
                      value={newMessage}
                      onChange={(e) => setNewMessage(e.target.value)}
                      onKeyDown={handleKeyPress}
                      rows={1}
                    />
                    <button
                      onClick={sendMessage}
                      disabled={!newMessage.trim()}
                      className="bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-xl px-5 md:px-6 py-3 disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base font-medium hover:shadow-lg transition-all duration-200 flex items-center justify-center min-w-[80px] shadow-sm"
                    >
                      Send
                    </button>
                  </div>
                </div>
              </>
            ) : activeTab === MESSAGE_TYPES.ADMIN ? (
              <>
                {/* Admin Header */}
                {(!isMobile || !isChatFullscreen) && (
                  <div className="bg-white p-4 border-b border-gray-200">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg flex items-center justify-center text-white shadow-sm">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>
                      </div>
                      <div>
                        <h3 className="font-bold text-gray-800">Administration</h3>
                        <p className="text-gray-600 text-sm">Support and assistance</p>
                      </div>
                    </div>
                  </div>
                )}
                
                {/* Messages Area */}
                <div 
                  ref={chatContainerRef}
                  className="flex-1 overflow-y-auto bg-gradient-to-b from-gray-50 to-white"
                >
                  <div className="p-3 md:p-4">
                    {messages.length === 0 ? (
                      <EmptyState activeTab={activeTab} />
                    ) : (
                      <div className="max-w-3xl mx-auto space-y-4 md:space-y-6">
                        {Object.entries(messageGroups).map(([date, dateMessages]) => (
                          <div key={date} className="space-y-2 md:space-y-3">
                            <DateSeparator date={date} />
                            <div className="space-y-2">
                              {dateMessages.map(msg => (
                                <MessageBubble
                                  key={msg._id}
                                  message={msg}
                                  isOwn={msg.sender === staff._id}
                                  isUnread={isMessageUnread(msg._id)}
                                  activeTab={activeTab}
                                  staff={staff}
                                  formatTime={formatTime}
                                />
                              ))}
                            </div>
                          </div>
                        ))}
                        <div ref={messagesEndRef} />
                      </div>
                    )}
                  </div>
                </div>

                {/* Message Input */}
                <div className="bg-white border-t border-gray-200 p-3 md:p-4">
                  <div className="flex space-x-2 md:space-x-3">
                    <textarea
                      ref={textareaRef}
                      placeholder="Message administration..."
                      className="flex-1 bg-gray-50 border-2 border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 text-sm md:text-base resize-none transition-all duration-200"
                      value={newMessage}
                      onChange={(e) => setNewMessage(e.target.value)}
                      onKeyDown={handleKeyPress}
                      rows={1}
                    />
                    <button
                      onClick={sendMessage}
                      disabled={!newMessage.trim()}
                      className="bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl px-5 md:px-6 py-3 disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base font-medium hover:shadow-lg transition-all duration-200 flex items-center justify-center min-w-[80px] shadow-sm"
                    >
                      Send
                    </button>
                  </div>
                </div>
              </>
            ) : (
              /* Empty State - Only show when not in fullscreen chat */
              !isChatFullscreen && (
                <div className="flex-1 flex items-center justify-center bg-gradient-to-b from-gray-50 to-white">
                  <div className="text-center text-gray-500 max-w-md mx-auto p-6">
                    <svg className="w-20 h-20 mx-auto mb-6 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z" />
                    </svg>
                    <h3 className="text-xl font-medium text-gray-700 mb-2">Select a Conversation</h3>
                    <p className="text-gray-600 mb-6">
                      Choose a resident to message, or contact administration for support.
                    </p>
                    {isMobile && !isSidebarOpen && (
                      <button
                        onClick={() => setIsSidebarOpen(true)}
                        className="bg-gradient-to-r from-red-500 to-orange-500 text-white px-6 py-2.5 rounded-lg font-medium hover:shadow-lg transition-all duration-200 shadow-sm"
                      >
                        View Conversations
                      </button>
                    )}
                  </div>
                </div>
              )
            )}
          </div>
        </div>
      </div>
    </>
  );
}

export default StaffMessages;