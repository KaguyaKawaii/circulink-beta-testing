...
function StaffMessages({ setView, staff, onLogout }) {
  ...
  // 🆕 FIXED: Mark messages as read for a specific user
  ...
  // 🆕 FIXED: Mark admin messages as read
  ...
  // 🆕 FIXED: Update unread counts locally when staff sends a message
  ...
  useEffect(() => {
    ...
    // 🆕 FIXED: Handle when conversation is marked as read
  ...
  const fetchUserConversation = async (user) => {
    try {
      ...
      // 🆕 FIXED: Mark messages as read immediately when opening conversation
  ...
  // 🆕 FIXED: Improved sendMessage function with proper unread handling
  const sendMessage = async () => {
    ...
    if (activeTab === "floor" && selectedUser) {
      ...
      try {
        ...
        // 🆕 FIXED: Update unread counts immediately
