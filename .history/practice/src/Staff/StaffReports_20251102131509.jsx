...
function StaffReports({ setView, staff }) {
  ...
  const getStatusBadge = (status) => {
    ...
    return `${config.bg} ${config.text} border ${config.border} inline-flex items-center px-3 py-1 rounded-full text-sm font-medium`;
  };

  const getStatusConfig = (status) => {
    const config = getStatusBadge(status).split(' ');
    return {
      color: `${config[0]} ${config[1]} ${config[2]} ${config[3]}`,
      dot: config[4]
    };
  };
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-600">Report created</span>
                    <span className="text-sm text-gray-400 ml-auto">{formatPHDateTime(report.createdAt)}</span>
                  </div>
                  {report.startedAt && (
                    <div className="flex items-center gap-3">
                      <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                      <span className="text-sm text-gray-600">Work started</span>
                      <span className="text-sm text-gray-400 ml-auto">{formatPHDateTime(report.startedAt)}</span>
                    </div>
                  )}
                  {report.resolvedAt && (
                    <div className="flex items-center gap-3">
                      <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                      <span className="text-sm text-gray-600">Report resolved</span>
                      <span className="text-sm text-gray-400 ml-auto">{formatPHDateTime(report.resolvedAt)}</span>
                    </div>
                  )}
                </div>
              </div>
