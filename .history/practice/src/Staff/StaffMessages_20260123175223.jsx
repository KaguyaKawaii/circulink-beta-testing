...
function StaffMessages({ setView, staff, onLogout }) {
  ...
  useEffect(() => {
    ...
    const handleConversationUnreadUpdate = (data) => {
      console.log("Conversation unread update:", data);
  ...
  if (activeTab === "floor" && selectedUser) {
    ...
    try {
      // 1. Send the reply to the server
      ...
      // 2. CRITICAL FIX: Immediately clear the unread badge in the local sidebar state
      setConversations(prev => 
        prev.map(conv => 
          conv._id === selectedUser._id 
            ? { ...conv, unreadCount: 0 } 
      ...
      // Update selectedUser state so the count is zeroed out
      setSelectedUser(prev => prev ? { ...prev, unreadCount: 0 } : null);
      
      // 3. Double-check synchronization with a mark-read call
      await axios.put(`${import.meta.env.VITE_API_URL}/api/messages/mark-read`, {
        staffId: staff._id,
        targetUserId: selectedUser._id
      });
      
      // 4. Refresh the global unread count badge
      fetchTotalUnreadCount();
