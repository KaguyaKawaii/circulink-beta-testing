...
const Log = require("../models/Log"); // Changed from AdminLog to Log
...
exports.getAnalyticsOverview = async (req, res) => {
  try {
    ...
    const [
      ...
      // Engagement stats - Modified to work with Log model
      Promise.all([
        ...
        // For session duration - you might need to calculate this differently or use 0 for now
        Promise.resolve([{ avgDuration: 0 }])
      ])
    ]);
    ...
    const analyticsData = {
      ...
      engagement: {
        ...
        averageSession: 0, // Set to 0 for now since Log doesn't have duration
...
exports.exportAnalytics = async (req, res) => {
  try {
    ...
    const overview = await exports.getAnalyticsOverview({ query: { range } }, { 
      json: (data) => data,
      status: () => ({ json: () => {} })
    });
    ...
    const reservations = await exports.getReservationAnalytics({ query: { range } }, { 
      json: (data) => data,
      status: () => ({ json: () => {} })
    });
    
    const rooms = await exports.getRoomAnalytics({ query: { range } }, { 
      json: (data) => data,
      status: () => ({ json: () => {} })
    });
...
async function getRecentActivity(limit = 10) {
  const logs = await Log.find() // Using Log instead of AdminLog
    .sort({ createdAt: -1 })
    .limit(limit)
...
async function getActiveUsersData(range) {
  ...
  return await User.aggregate([
    { $match: { lastLogin: { $gte: new Date(Date.now() - days * 24 * 60 * 60 * 1000) } } },
    { $group: {
