...
/* ------------------------------------------------
   ✅ ADMIN CREATE RESERVATION (AUTO-APPROVED) - FIXED VERSION
------------------------------------------------ */
exports.adminCreateReservation = async (req, res) => {
  try {
    ...
    for (const participant of participants) {
      ...
      if (!participant.id_number || !participant.id_number.trim()) {
        continue;
      }
    ...
    if (enrichedParticipants.length !== parseInt(numUsers)) {
      return res.status(400).json({
        message: `Participant count mismatch. Expected ${numUsers} valid participants, but found ${enrichedParticipants.length}.`
      });
    }
    ...
    console.log("✅ Admin reservation created:", reservation._id);
