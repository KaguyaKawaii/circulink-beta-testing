...
/* ------------------------------------------------
   ✅ ADMIN CREATE RESERVATION (AUTO-APPROVED) - FIXED WITH DEBUGGING
------------------------------------------------ */
exports.adminCreateReservation = async (req, res) => {
  try {
    ...
    console.log("📝 ADMIN CREATE RESERVATION REQUEST");
    console.log("=".repeat(50));
    ...
    if (!numUsers) missingFields.push("numUsers");
    ...
    // Calculate end datetime if not provided
    ...
    if (endDatetime) {
      try {
        ...
        console.log("❌ Invalid endDatetime format:", endDatetime);
        return res.status(400).json({ message: "Invalid endDatetime format" });
      }
    } else {
      // Default to 2 hours
      parsedEndDatetime = new Date(parsedDatetime.getTime() + 120 * 60 * 1000);
    }
    ...
    try {
      ...
      console.log("✅ Main user found:", user.name, user.email);
    } catch (err) {
    ...
    try {
      ...
      if (conflictingReservation) {
        console.log("❌ Room conflict found:", conflictingReservation._id);
        return res.status(400).json({ 
    ...
    for (const [index, participant] of participants.entries()) {
      ...
      if (!participant.id_number || !participant.id_number.toString().trim()) {
        console.log(`⚠️ Participant ${index + 1} has no ID number, skipping`);
        continue;
      }

      const idNumber = participant.id_number.toString().trim();
      ...
      try {
        ...
        if (!participantUser) {
          console.log(`❌ Participant ${idNumber} not found in database`);
          invalidParticipants.push({
        ...
        console.log(`✅ Found participant: ${participantUser.name} (${participantUser.role})`);

        if (!participantUser.verified) {
          console.log(`❌ Participant ${participantUser.name} is not verified`);
          invalidParticipants.push({
            name: participantUser.name,
            id_number: participantUser.id_number,
            reason: "User account is not verified"
        ...
        if (conflicts.length > 0) {
          console.log(`❌ Participant ${participantUser.name} has conflicting reservation`);
          invalidParticipants.push({
            name: participantUser.name,
            id_number: participantUser.id_number,
        ...
        console.error(`❌ Error processing participant ${idNumber}:`, err);
        invalidParticipants.push({
          id_number: idNumber,
          reason: "Error processing participant: " + err.message
        });
      }
    }

    console.log(`✅ Enriched ${enrichedParticipants.length} participants`);
    console.log(`❌ Found ${invalidParticipants.length} invalid participants`);

    // Check if we have any invalid participants
    if (invalidParticipants.length > 0) {
    ...
    const expectedCount = parseInt(numUsers);
    if (enrichedParticipants.length !== expectedCount) {
      console.log(`❌ Participant count mismatch. Expected ${expectedCount}, got ${enrichedParticipants.length}`);
      return res.status(400).json({
        message: `Participant count mismatch. Expected ${expectedCount} valid participants, but found ${enrichedParticipants.length}.`
      });
    }
    ...
    try {
      ...
      console.error("❌ Error creating reservation in database:", err);
      ...
      return res.status(500).json({ 
        message: "Database error creating reservation",
        error: err.message 
      });
    }
    ...
    // Notify main user (don't fail if notification fails)
    try {
      await notificationService.createNotification(
        {
          userId,
      ...
      console.log("✅ Notification sent to main user");
    } catch (notifError) {
      console.warn("⚠️ Failed to notify main user:", notifError.message);
    }

    // Notify participants (don't fail if notifications fail)
    for (const participant of enrichedParticipants) {
      if (participant.id_number === user.id_number) continue;
      ...
      } catch (partNotifError) {
        console.warn(`⚠️ Failed to notify participant ${participant.id_number}:`, partNotifError.message);
      }
    }
    ...
    console.error("❌ FATAL ERROR IN ADMIN CREATE RESERVATION");
    console.error("=".repeat(50));
    ...
    res.status(500).json({ 
      message: "Internal server error creating reservation",
...
exports.editReservation = async (req, res) => {
  try {
    ...
    if (!updatedReservation) {
      return res.status(404).json({ message: "Reservation not found after update" });
    }

    console.log("✅ Reservation updated successfully:", updatedReservation._id);
